...
- CodeBuild project
  ...
  - エージェント削除用
- AgentCore Runtime 用コンテナイメージの ECR リポジトリ
- CodeBuild サービスロール
- CodeBuild ロググループ
...
6. `パラメータ` セクションにて 必要な値を入力し [次へ] をクリック
    ...
    | パラメータ名                     | 説明                                                                                  | 入力例・補足                                                                                                                                                                                                                                     |
    | :------------------------------- | :------------------------------------------------------------------------------------ | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
    | **AgentImageUrl**                | TracisAI Agent 用の ECR イメージ URL                                                  | 通常は空のまま使用してください。空の場合は、このリリースで定義された標準イメージをデプロイ先リージョンの ECR にコピーして使用します。独自イメージを使用する場合のみ ECR イメージ URL を指定してください。                                     |
    ...
    | **Prefix**                       | すべてのリソースに対するプレフィックス                                                | `my-app`<br>※ S3 バケット名にも使用します。英小文字、数字、ハイフンのみを使用し、先頭・末尾をハイフンにしない 28 文字以内の値を入力してください。この条件に合わない値では、CloudFormation でスタックを作成できません。                           |
