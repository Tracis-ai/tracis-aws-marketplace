module "firelens_config_bucket" {
  ...
  bucket        = "${var.prefix}-tracis-firelens-config-${data.aws_caller_identity.current.account_id}"
  force_destroy = true
  ...
  tags = {
    Name = "${var.prefix}-tracis-firelens-config-${data.aws_caller_identity.current.account_id}"
  }
}
